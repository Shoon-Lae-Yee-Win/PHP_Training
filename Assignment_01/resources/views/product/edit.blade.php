            ...
            <div class="card col-md-6 shadow offset-3 my-5 rounded-3 px-5 py-3">
                ...
                <form action="{{ route('product#update', $product->id) }}" method="POST">
                    ...
                    @error('prod_name') is-invalid @enderror"
                            value="{{ old('prod_name', $product->prod_name) }}" placeholder="Enter product name ...">
                        @error('prod_name')
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="category" id="category" class="form-control @error('category') is-invalid @enderror">
                            ...
                            @foreach ($categories as $c)
                                <option value="{{ $c->id }}"
                                    {{ old('category', $product->cat_id) == $c->id ? 'selected' : '' }}>{{ $c->cat_name }}
                                </option>
                            @endforeach
                        </select>
                        @error('category')
                    ...
                    <div class="form-group mb-3">
                        <button class="btn"><a href="{{ route('product#list') }}">Cancel</a></button>
